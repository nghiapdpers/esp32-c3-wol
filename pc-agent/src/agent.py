...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        topic_status = f"esp32_c3_wol/{userdata['secret_key']}/status/{my_mac.replace(':', '')}"
        client.publish(topic_status, "online", retain=True)
        print(f"🟢 Đã gửi trạng thái Online tới: {topic_status}")
    else:
...
def on_message(client, userdata, msg):
    try:
        ...
        if cmd == "shutdown" and mac_target.upper() == my_mac.upper():
...
def main():
    ...
    config["override_mac"] = my_mac # Lưu vào userdata để dùng trong callback
    ...
    try:
        ...
        for interface, mac in all_macs:
            print(f"  - {interface}: {mac} {' (Đang dùng)' if mac == my_mac else ''}")
    except Exception as e:
        print(f"  ⚠️ Không thể liệt kê đầy đủ card mạng: {e}")
    
    print(f"\n📍 MAC Address đang sử dụng: {my_mac}")
    ...
    topic_status = f"esp32_c3_wol/{config['secret_key']}/status/{my_mac.replace(':', '')}"
    ...
    while True:
        try:
            ...
            print(f"🔄 Đang kết nối lại sau 5s... ({e})")
            time.sleep(5)
